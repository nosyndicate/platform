...
class Box():
	...
	# 	denom = ((endy - starty) * (self.x_pos_right - self.x_pos_left)) - ((endx - startx) * (self.y_pos_right - self.y_pos_left))
...
class TwoLaneEnv(gym.Env):
	...
	def __init__(self, sameSide):
		...
		if sameSide:
			...
			# right ball
			self.balls.append(Ball(self.max_step, self.field, self.field.width - self.circleDiameter, self.circleDiameter, self.circleDiameter, 1))


			self.num_boxes = 2;
			self.boxes = []
			# left box (0, height/2) (width / 2 - passageWidth / 2, height/2)
			self.boxes.append(Box(self.field, 0, self.field.height / 2., self.field.width / 2. - self.passageWidth / 2.0, self.field.height / 2., self.BoxHeight))
			# right box (width / 2 + passageWidth / 2, height/2) (width, height/2)
			self.boxes.append(Box(self.field, self.field.width / 2. + self.passageWidth / 2.0, self.field.height / 2., self.field.width, self.field.height / 2., self.BoxHeight))
			
			
			self.num_agents = self.get_num_agents()

			self.action_space = spaces.Box(low=-1.,high=1., shape=(self.num_balls,2))
			self.observation_space = spaces.Box(0,self.field.width, shape=(self.num_balls,2))
		else:
			...
			self.boxes.append(Box(self.field, 0, self.field.height / 2. - self.BoxHeight / 2., self.field.width / 2. - self.passageWidth / 2.0, self.field.height / 2. - self.BoxHeight / 2., self.BoxHeight))
